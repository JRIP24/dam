...
public class Clinic {
	...
	public String bookVisit(Animal a, boolean discount) {
		...
		while (!foundDate) {
			...
			if (a instanceof Pet) {
				if (!bookedDay(date, 'D') && !bookedDay(date, 'M')) {
					shift = 'M';
					foundDate = true;
				} else if (!bookedDay(date, 'D') && !bookedDay(date, 'A')) {
					shift = 'A';
					foundDate = true;
				}
			} else {
	...
	public HashSet<Animal> visitedAnimals(final String type) {
		...
		HashSet<Animal> animales = new HashSet<Animal>();
		...
		} else { //Se supondrá que si no es pet será farm
